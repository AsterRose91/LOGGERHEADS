...
var _gameOver = false;
with (objPlayer_SP){
	_gameOver = (STATE == CHARACTER_STATES.OOPS);	
}
if (_gameOver){
...
DRAW_HUD = !(getRoomType(room) == ROOMTYPES.FRIEND_HOUSE || getRoomType(room) == ROOMTYPES.YOUR_HOUSE || getRoomType(room) == ROOMTYPES.HATMAN_ENCOUNTER);
...
if (instance_exists(objParentMenu)) {
	...
	// IF NO MENU OBJECTS EXIST, NO PAUSING
	GAME_PAUSED = false;
}
